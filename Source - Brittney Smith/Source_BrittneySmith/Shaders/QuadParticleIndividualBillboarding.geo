...
void main()
{
	// Individual billboarding - calculate values
	vec3 CameraFront = normalize(gl_in[0].gl_Position.xyz - CameraPosition);

	vec3 QuadAxisX = cross(CameraFront, CameraUpDir);
	QuadAxisX = normalize(QuadAxisX);

	vec3 QuadAxisY = cross(CameraFront, QuadAxisX);
	QuadAxisY = normalize(QuadAxisY);
